...
Commands: `pnpm dev`, `pnpm supervisor <config.json>`, `pnpm verify` (fast deterministic gate: the repair-loop checks), `pnpm verify:full` (`verify` + the self Visual QA catalog smoke; the closing gate for anything that could be activated), `pnpm test` (unit only), `pnpm test:integration`, `pnpm test:visual-catalog`, `pnpm test:all`, `pnpm test:e2e`. Live agents require explicit `JARVIS_LIVE_AGENT_TESTS=1`.
