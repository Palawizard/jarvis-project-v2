...
6. Configured web projects run on isolated dynamic ports, and an interactive QA agent drives a confined browser against them: its checkpoints are the evidence. Before it starts, Jarvis derives the mandatory checks from the request itself — one per declared viewport, one per acceptance criterion — and the gate is deterministic: a required check that is absent, unreached, or unphotographed is `qa_inconclusive`, never a pass. Findings at a configured blocking severity (`JARVIS_VISUAL_REVIEW_BLOCKING_SEVERITIES`, default high/medium) enter a bounded visual repair → verification → code review → recapture loop; the rest are recorded as advisories and shown before approval.
