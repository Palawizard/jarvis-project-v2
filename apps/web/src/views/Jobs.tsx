...
export function JobsView({
  ...
  const table = (list: Job[]) => (
    <table>
      ...
      <tbody>
        {list.map((job) => (
          <tr key={job.id} className="clickable" onClick={() => onOpen(job.id)}>
            <td>
              <div>{job.goal}</div>
              {job.error && <div className="tiny" style={{ color: 'var(--err)' }}>{job.error}</div>}
            </td>
            <td className="small dim nowrap">{byId.get(job.projectId)?.name ?? '—'}</td>
            <td><StageBadge stage={job.stage} /></td>
  ...
  return (
    <div className="page">
      {awaiting.length > 0 && <Card title={`Awaiting your decision (${awaiting.length})`}>{table(awaiting)}</Card>}
