...
- The existing tool-free Brief Compiler also returns non-authoritative `executionRecommendation` advice for the initial implementer: `normal|strong` capability, `low|medium|high` effort, and 1-4 concrete reasons. This is the same structured-output call that compiles the brief, not a selector run. When valid, it replaces request length and brief-count scoring; trusted self-development/high-risk floors, security floors, role bounds, provider health and the fixed provider mapping still win. Missing or malformed advice falls back to deterministic policy. Reviewer, fixer and Visual QA continue to route from post-implementation evidence.
