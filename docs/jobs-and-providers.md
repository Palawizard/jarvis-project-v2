...
Projects may define ordered `verification.steps` with `name`, `command`, `timeoutMs`, `required`, and `kind`. Setup/install is recorded separately from evidence. Reports classify `none`, `product`, `infrastructure`, or `cancelled`: setup failure, spawn/start failure, missing executable, and timeout are infrastructure; an ordinary completed check with a non-zero exit is product failure. Infrastructure retries are bounded by `JARVIS_VERIFICATION_INFRA_RETRIES` (default 2) and never invoke a source fixer. Jarvis executes the trusted-project commands itself, shows exactly what ran, and cannot call unit-only evidence “all tests.” For Jarvis, `pnpm verify` covers format, lint, typecheck, all non-live Vitest projects (unit and integration), build, and Playwright E2E. Live subscription-agent tests remain opt-in.
